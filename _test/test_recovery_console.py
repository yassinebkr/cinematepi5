...
class ConfigLadderTests(TempCase):
    ...
    def test_reason_explains_the_fallback(self):
        ...
        self.assertIn("settings.json unusable", self.load().reason)
...
class WriteDisciplineTests(TempCase):
    ...
    def test_rotation_keeps_ten_and_never_the_oldest(self):
        ...
        for i in range(1, 16):
            (self.backups / f"settings.json.202601{i:02d}T000000Z.bak").write_text(
                f"gen{i}", encoding="utf-8"
            )
        rc.prune_backups("settings.json", self.backups, keep=10)
        survivors = sorted(p.name for p in self.backups.iterdir())
        self.assertEqual(len(survivors), 10)
        self.assertIn("settings.json.20260101T000000Z.bak", survivors)  # oldest
        self.assertIn("settings.json.20260115T000000Z.bak", survivors)  # newest
        # The pristine original is still readable, ten bad edits later.
        self.assertEqual(
            (self.backups / "settings.json.20260101T000000Z.bak").read_text(), "gen1"
        )

    def test_rotation_leaves_a_small_set_alone(self):
        self.backups.mkdir(parents=True)
        for i in range(3):
            (self.backups / f"settings.json.2026010{i}T000000Z.bak").write_text("x")
        self.assertEqual(rc.prune_backups("settings.json", self.backups, 10), [])
        self.assertEqual(len(list(self.backups.iterdir())), 3)

    def test_rotation_is_per_file(self):
        self.backups.mkdir(parents=True)
        for i in range(12):
            (self.backups / f"settings.json.2026010{i:02d}T000000Z.bak").write_text("s")
        for i in range(3):
...
class AnsiRenderingTests(unittest.TestCase):
    ...
    def test_real_config_loader_output_renders(self):
        ...
        err = SettingsLoadError(
            Path("/x/settings.json"), "summary", "detail at line 3, column 5",
