...
class SensorDatabaseTests(unittest.TestCase):
    ...
    def test_packing_for_platform_is_data_driven(self):
        ...
        # A sensor with no packing_by_platform override must keep its own
        # database/default packing on both platforms.  Do not hard-code a
        # particular IMX283 packing here: that value is driver-generation
        # specific and belongs in resources/sensors.json.
        imx283_default = d.get_packing("imx283", 0)
        self.assertEqual(d.get_packing_for_platform("imx283", 0, is_pi4=True), imx283_default)
        self.assertEqual(d.get_packing_for_platform("imx283", 0, is_pi4=False), imx283_default)
...
"""
    ...
    def _detector_for_parse(self):
        detector = self._detector_without_probe()
        # Parser tests must not inherit the operator's live resolution filter
        # from src/settings.json.  Keep every 10/12-bit fixture mode so the
        # test remains deterministic on cameras configured for 3K/4K only.
        detector.settings = {}
        detector.k_steps = []
        detector.bit_depths = [10, 12]
