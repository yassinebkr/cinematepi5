...
class CinePiController:
    ...
    def set_pwm_mode(self, state):
        if self.current_sensor != 'imx477':
            logging.info(f"Current sensor {self.current_sensor}. PWM mode not available")
        else:
            if self.pwm_mode == False:
                self.fps_saved = float(self.get_setting('fps_actual'))

            self.pwm_mode = state
            logging.info(f"Setting pwm mode to {state}")
    ...
    #         logging.info(f"Setting pwm mode to {pwm_mode}")
    ...
    def set_white_balance(self, wb_value):
        if wb_value in self.wb_steps:
            self.redis_controller.set_value('wb', wb_value)
            logging.info(f"Setting white balance to {wb_value}")
        else:
            logging.error(f"Invalid white balance value: {wb_value}")

    def inc_wb(self):
        current_wb = int(self.get_setting('wb'))
        current_index = self.wb_steps.index(current_wb)
        if current_index < len(self.wb_steps) - 1:
            new_wb = self.wb_steps[current_index + 1]
            self.set_white_balance(new_wb)
    ...
    def toggle_auto_wb(self):
        current_auto_wb = self.get_setting('auto_wb')
        new_auto_wb = "0" if current_auto_wb == "1" else "1"
        self.redis_controller.set_value('auto_wb', new_auto_wb)
        logging.info(f"Auto white balance set to {'on' if new_auto_wb == '1' else 'off'}")

    def set_ev(self, ev_value):
        self.redis_controller.set_value('ev', str(ev_value))
        logging.info(f"Setting EV to {ev_value}")

    def inc_ev(self):
        current_ev = float(self.get_setting('ev'))
        new_ev = min(current_ev + 0.1, 2.0)  # Assuming max EV is 2.0
        self.set_ev(round(new_ev, 1))

    def dec_ev(self):
        current_ev = float(self.get_setting('ev'))
        new_ev = max(current_ev - 0.1, -2.0)  # Assuming min EV is -2.0
        self.set_ev(round(new_ev, 1))

    def toggle_auto_ev(self):
        current_auto_ev = self.get_setting('auto_ev')
        new_auto_ev = "0" if current_auto_ev == "1" else "1"
        self.redis_controller.set_value('auto_ev', new_auto_ev)
        logging.info(f"Auto EV set to {'on' if new_auto_ev == '1' else 'off'}")
