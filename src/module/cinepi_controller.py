...
class CinePiController:
    ...
    def _resolution_change_needs_restart(self, target_mode):
        ...
        try:
            ...
            cur_bd = self.redis_controller.get_value(ParameterKey.BIT_DEPTH.value)
            if cur_bd is not None and new_bd != int(float(cur_bd)):
                return True
    ...
    # ------------------------------------------------------------------
    # Guided IMU calibration UI / physical-control routing
    # ------------------------------------------------------------------
    def _imu_cal_active(self):
        try:
            raw = self.redis_controller.r.get('imu_cal_active')
    ...
    def _imu_cal_status(self):
        import json as _json
        try:
            raw = self.redis_controller.r.get('imu_cal_status')
            if not raw:
                return {}
            if isinstance(raw, (bytes, bytearray)):
                raw = raw.decode()
    ...
    def _imu_cal_send(self, action, **extra):
        import json as _json
        import time as _time
        payload = {'id': str(_time.time_ns()), 'action': str(action)}
        payload.update(extra)
        self.redis_controller.r.set('imu_cal_cmd', _json.dumps(payload))
        return payload['id']

    def _imu_cal_set_notice(self, text):
        try:
            self.redis_controller.r.set('imu_cal_notice', str(text))
        except Exception:
            pass

    def start_imu_calibration(self):
        """WB encoder hold: enter calibration; hold again to cancel."""
        if self._imu_cal_active():
            self._finish_imu_calibration(save=False)
            return True

        if self._is_recording():
            self._imu_cal_set_notice('Stop recording before IMU calibration.')
            logging.warning('IMU calibration refused while recording')
            return False

        try:
            self.redis_controller.r.mset({
                'imu_cal_active': '1',
                'imu_cal_selection': '1',  # default result selection = SAVE
                'imu_cal_view_yaw': '25',
                'imu_cal_notice': '',
            })
            self._imu_cal_send('start_session')
            # The C++ DRM preview is intentionally disabled during calibration so
            # the full 1920x1080 framebuffer is available to the wizard.
            self.restart_camera(preview_enabled=False)
            logging.info('IMU calibration wizard started')
            return True
        except Exception:
            logging.exception('Failed to start IMU calibration')
            try:
                self.redis_controller.r.set('imu_cal_active', '0')
            except Exception:
                pass
            return False

    def _finish_imu_calibration(self, save=False):
        import time as _time
        action = 'save' if save else 'cancel'
        try:
            self._imu_cal_send(action)
            wanted = 'saved' if save else 'cancelled'
            deadline = _time.monotonic() + 3.0
            while _time.monotonic() < deadline:
                if self._imu_cal_status().get('phase') == wanted:
                    break
                _time.sleep(0.05)
        except Exception:
            logging.exception('IMU calibration finish handshake failed')
        finally:
            try:
                self.redis_controller.r.set('imu_cal_active', '0')
            except Exception:
            ...
            try:
                ...
                logging.exception('Failed to restore preview after IMU calibration')
        return True

    def _imu_cal_input(self, action):
        """Route encoder/button actions while the full-screen wizard is active."""
        if not self._imu_cal_active():
            return False

        status = self._imu_cal_status()
        phase = str(status.get('phase', 'intro'))
        can_continue = bool(status.get('can_continue', False))

        if action in ('view_left', 'view_right'):
            try:
                raw = self.redis_controller.r.get('imu_cal_view_yaw')
                yaw = float(raw.decode() if isinstance(raw, (bytes, bytearray)) else (raw or 25))
                yaw += -12.0 if action == 'view_left' else 12.0
                self.redis_controller.r.set('imu_cal_view_yaw', '%.1f' % yaw)
            except Exception:
                pass
            return True
        ...
        if action in ('next', 'prev'):
            if phase == 'results':
                try:
                    raw = self.redis_controller.r.get('imu_cal_selection')
                    sel = int(raw or 1)
                except Exception:
                    sel = 1
                sel = (sel + (1 if action == 'next' else -1)) % 3
                self.redis_controller.r.set('imu_cal_selection', str(sel))
            return True

        if action == 'back':
            self._imu_cal_set_notice('Hold the WB button for 2 seconds to cancel calibration.')
            return True
        ...
        if phase == 'intro':
            self._imu_cal_send('start_gyro')
        elif phase == 'gyro_done':
            self._imu_cal_send('start_tumble')
        elif phase == 'tumble':
            if can_continue:
                self._imu_cal_send('finish_tumble')
            else:
                self._imu_cal_set_notice('Keep rotating until coverage is sufficient.')
        elif phase == 'accel_done':
            self._imu_cal_send('start_faces')
        elif phase == 'faces':
            if can_continue:
                self._imu_cal_send('capture_face')
        elif phase == 'results':
            try:
                raw = self.redis_controller.r.get('imu_cal_selection')
                sel = int(raw or 1)
                ...
                sel = 1
            if sel == 0:
                self._imu_cal_send('retry')
                self.redis_controller.r.set('imu_cal_selection', '1')
            elif sel == 1:
                self._finish_imu_calibration(save=True)
            else:
                self._finish_imu_calibration(save=False)
        return True

    def inc_shutter_a(self):
        if self._imu_cal_input('next'):
            return
        self.increment_setting('shutter_a', self.shutter_a_steps, fps=self.fps)

    def dec_shutter_a(self):
        if self._imu_cal_input('prev'):
            return
    ...
    def inc_wb(self):
        if self._imu_cal_input('view_right'):
            return
        self.set_wb(direction='next')

    def dec_wb(self):
        if self._imu_cal_input('view_left'):
            return
    ...
    def set_zoom(self, value=None, direction="next"):
        if value is None and self._imu_cal_input('back'):
            return
        """
