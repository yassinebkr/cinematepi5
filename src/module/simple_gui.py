...
from module import imu_calibration_ui
import json
import re
...
class SimpleGUI(threading.Thread):
    def __init__(self, 
        ...
        # The camera DRM plane runs independently at sensor cadence. Keep the
        # framebuffer/HUD close to it so overlays feel live. The RGB565 fast
        # path in framebuffer.py keeps this within the Pi 5 CPU budget.
        self.target_fps = 30
        self.min_frame_interval = 1 / self.target_fps
        self.slow_refresh_interval = 1.0
        self._redraw_event = threading.Event()
        self._last_drawn_values = None
        self._last_vu_sig = None
    ...
    def _hud_level_active(self):
        try:
            rc = getattr(self, '_hud_rc', None)
            if rc is None:
                import redis as _r; rc = _r.Redis(); self._hud_rc = rc
            return (rc.get('hud_level') or b'0').decode() == '1'
        except Exception:
            return False
        
    def draw_level_hud(self, draw):
        import math as _m
        rc = getattr(self, '_hud_rc', None)
        if rc is None:
            import redis as _r; rc = _r.Redis(); self._hud_rc = rc
        ...
        try:
            rect = _calculate_preview_guide_rect(w, h, self.width or 1920, self.height or 1080)
            x0 = rect[2] + 4
        except Exception:
            x0 = w - 150
        sw = w - x0
        if sw < 70:
            x0 = w - 150; sw = 150
        cx, cy = x0 + sw // 2, h // 2 - 80
        R = max(26, min(52, sw // 2 - 12))
        if not getattr(self, '_hud_logged', False):
            logging.info('level HUD drawing at (%s,%s) roll=%.1f shake=%.1f', cx, cy, roll, shake)
            self._hud_logged = True
        ok = abs(roll) < 0.7
        ...
        draw.line([(cx + R - 10, cy), (cx + R, cy)], fill=(170, 170, 170, 255), width=2)
        font = getattr(self, '_hud_font', None)
        if font is None:
            try:
                font = ImageFont.truetype(os.path.realpath(self.font_path), 24)
            except Exception:
        ...
        # angular speed, so use a more sensitive scale.
        sc = (60, 255, 100, 255) if shake < 1.5 else ((255, 210, 60, 255) if shake < 5.0 else (255, 70, 70, 255))
        draw.rectangle((bx, by, bx + int(bw * min(1.0, shake / 12.0)), by + bh2), fill=sc)
        draw.rectangle((bx, by, bx + bw, by + bh2), outline=(200, 200, 200, 255))
        draw.text((bx + 14, by + 14), '%.0f\u00b0/s' % shake, fill=sc, font=font)

    def draw_recording_indicator(self, draw):
        """Blinking REC dot driven only by canonical IS_RECORDING."""
        try:
            active = int(
                self.redis_controller.get_value(ParameterKey.IS_RECORDING.value)
                or 0
            ) == 1
        except (TypeError, ValueError):
            active = False
        ...
        try:
            ...
            x0 = rect[2] + 4
            sw = w - x0
            cx = x0 + sw // 2 if sw >= 50 else w - 24
        except Exception:
            cx = w - 24

        cy = max(24, int(h * 0.065))
        radius = max(9, min(15, int(min(w, h) * 0.012)))
        bright = (time.monotonic() % 1.0) < 0.58
        fill = (255, 38, 38, 255) if bright else (78, 8, 8, 255)
        ring = (255, 125, 125, 255) if bright else (120, 35, 35, 255)
    ...
    def draw_gui(self, values):
        if imu_calibration_ui.is_active(self):
            imu_calibration_ui.draw(self)
            return
        ...
        if not preroll_active and not drop_frame_live and not sync_flash_live:
            if int(self.redis_controller.get_value(ParameterKey.IS_WRITING.value) or 0) == 1:
                # REC is shown by a dedicated IS_RECORDING indicator instead
                # of turning the entire HDMI framebuffer red.
                self.current_background_color = "black"
                self.color_mode = "normal"
        ...
        if self._hud_level_active():
            try:
                self.draw_level_hud(draw)
            except Exception:
                logging.exception('level HUD draw failed')

        try:
            self.draw_recording_indicator(draw)
        except Exception:
            logging.exception('recording indicator draw failed')
    ...
    def draw_rounded_box(self, draw, text, position, font_size, padding, text_color, fill_color, image, extra_height=-17, reduce_top=12):
        ...
        # Top-left corner
        image.paste(fill_color, (int(upper_left[0]), int(upper_left[1])), mask)
        # Top-right corner
    ...
    def run(self):
        try:
            ...
            while self._running:
                ...
                _ha = self._hud_level_active()
                _cal = imu_calibration_ui.is_active(self)
                if _ha != getattr(self, '_hud_prev', None):
                    self._hud_prev = _ha
                    self._fast_dirty = True
                has_work = self._fast_dirty or self._slow_dirty or self._vu_active() or _ha or _cal
                if not has_work:
                    self._redraw_event.wait(timeout=0.1)
                    self._redraw_event.clear()
                    continue

                _frame_interval = (1.0 / 15.0) if _cal else (min(self.min_frame_interval, 0.1) if _ha else self.min_frame_interval)
                due_in = max(0.0, _frame_interval - (now - self._last_draw_ts))
                if due_in > 0:
                    time.sleep(due_in)
                    self._redraw_event.clear()
                    continue
                ...
                vu_sig = (tuple(int(round(v*64)) for v in (self.vu_smoothed or [])),
                          tuple(int(round(v*64)) for v in (self.vu_peaks or [])))
                if self._slow_dirty or vu_sig != self._last_vu_sig or values != self._last_drawn_values or self._fast_dirty or self._hud_level_active() or _cal:
                    try:
                        self.draw_gui(values)
                    except Exception:
                        logging.exception("draw_gui failed; skipping frame")
                    try:
                        self._last_drawn_values = dict(values)
                    except Exception:
                        self._last_drawn_values = values
                    self._last_vu_sig = vu_sig
