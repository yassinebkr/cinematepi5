...
class SmartButton:
    ...
    def __init__(self, cinepi_controller, pin, pull_up, debounce_time, actions, identifier, inverse=False, combined_actions=[]):
        ...
        self.defer_press_action = bool(actions.get('defer_press_action', False))
        self.hold_seconds = float(actions.get('hold_seconds', 3.0))
    ...
    def on_press(self):
        ...
        with self.lock:
            ...
            if not self.defer_press_action:
                self.trigger_action(self.actions.get('press_action'))
            
            # Start hold timer to check for hold action
            if self.hold_timer is not None:
                self.hold_timer.cancel()
            self.hold_timer = threading.Timer(self.hold_seconds, self.trigger_hold_action)
    ...
    def on_release(self):
        ...
        with self.lock:
            ...
            # For a deferred press, execute the ordinary click action only when
            # the hold action did not fire.
            if self.defer_press_action and not self.hold_fired:
                self.trigger_action(self.actions.get('press_action'))
