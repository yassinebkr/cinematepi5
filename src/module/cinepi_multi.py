...
class CinePiProcess(Thread):
    ...
    def _build_args(self):
        ...
        preview_cfg = _settings().get("preview", {}) or {}
        try:
            lores_max_height = int(preview_cfg.get("lores_max_height", 720) or 720)
        except (TypeError, ValueError):
            lores_max_height = 720
        lores_max_height = max(360, min(lores_max_height, ah))
        lh = min(lores_max_height, ah)
        lw = int(round(lh * aspect * anam))
        if lw > aw:
            lw, lh = aw, int(round(aw / (aspect * anam)))
        # YUV420 dimensions must stay even.
        lw = max(2, int(lw) & ~1)
        lh = max(2, int(lh) & ~1)
        ...
        logging.info(
            "[%s] Lores/web preview: %dx%d (cap=%d)",
            self.cam.port, lw, lh, lores_max_height,
        )
        if (aw/ah) > aspect:
        ...
        if tune_reason == "ok":
            ...
            logging.error(
                "[%s] Custom tuning file override NOT applied (%s); "
                "using auto-detected tuning %s",
                self.cam.port,
                tune_reason,
                tune,
            )
        ...
        # Keep MJPEG quality configurable from settings.json while retaining the
        # post-processing JSON as cinepi-raw's runtime configuration.
        try:
            mjpeg_quality = int(preview_cfg.get("mjpeg_quality", 85) or 85)
        except (TypeError, ValueError):
            mjpeg_quality = 85
        mjpeg_quality = max(30, min(95, mjpeg_quality))
        try:
            pp = {}
            if os.path.exists(post):
                with open(post, "r", encoding="utf-8") as f:
                    pp = json.load(f) or {}
            stage = pp.setdefault("mjpegPreview", {})
            if int(stage.get("quality", -1)) != mjpeg_quality:
                stage["quality"] = mjpeg_quality
                tmp = post + ".tmp"
                ...
                os.replace(tmp, post)
            logging.info(
                "[%s] MJPEG web quality: %d", self.cam.port, mjpeg_quality
            )
        except Exception:
            logging.exception("[%s] Failed to sync MJPEG quality", self.cam.port)
        ...
        # Pi 4 / VC4 uses its built-in tuning path.  Pi 5 / PiSP receives the
        # validated custom override or the auto-detected sensor tuning.
        if not self._is_pi4():
            args += ["--tuning-file", tune]
        elif self.tuning_file_override.get('enabled'):
            logging.info(
                "[%s] Custom tuning file override ignored on Pi 4/VC4",
                self.cam.port,
            )
...
class CinePiManager:
    ...
    def start_all(self, preview_enabled: Optional[bool] = None) -> None:
        ...
        # Hard invariant: the IMU calibration wizard owns the full HDMI
        # framebuffer. Never allow any caller/restart path to re-enable the DRM
        # preview plane while calibration is active.
        try:
            raw_cal = self.redis_controller.r.get("imu_cal_active")
            if isinstance(raw_cal, (bytes, bytearray)):
                raw_cal = raw_cal.decode()
            imu_cal_active = str(raw_cal or "0").strip().lower() in (
                "1", "true", "yes", "on"
            )
        except Exception:
            imu_cal_active = False

        if imu_cal_active:
            if self.preview_enabled:
                logging.warning(
                    "IMU calibration active: overriding requested preview ON -> OFF"
                )
