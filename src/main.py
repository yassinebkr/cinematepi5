...
def run_application(args, log_queue):
    ...
    #
    # Redis survives a CineMate process restart. If the IMU calibration wizard
    # was active, the first cinepi-raw process must be born with DRM preview
    # disabled; otherwise its hardware plane will cover the framebuffer wizard.
    try:
        _imu_cal_active_at_startup = str(
            redis_controller.r.get('imu_cal_active') or b'0',
            'utf-8'
        ).strip().lower() in ('1', 'true', 'yes', 'on')
    except Exception:
        _imu_cal_active_at_startup = False
    ...
    if _imu_cal_active_at_startup:
        logging.info(
            ...
            'starting cinepi-raw with DRM preview disabled'
        )
    ...
    if restart_camera_after_startup_handoff and camera_present:
        ...
        # calibration must come back with the DRM preview plane disabled.
        try:
            _imu_cal_active = str(
                redis_controller.r.get('imu_cal_active') or b'0',
                'utf-8'
            ...
            _imu_cal_active = _imu_cal_active_at_startup

        cinepi_controller.restart_camera(preview_enabled=not _imu_cal_active)
        if _imu_cal_active:
            logging.info(
