...
## Local camera rotary mapping

The current local IMX283 camera uses three direct GPIO rotary encoders:

| Control | CLK | DT | Button | Current state |
| --- | ---: | ---: | ---: | --- |
| Shutter | GPIO 23 | GPIO 24 | GPIO 25 | rotation usable; click currently unreliable |
| White balance | GPIO 6 | GPIO 13 | GPIO 5 | rotation usable; click currently unreliable |
| ISO | GPIO 17 | GPIO 27 | GPIO 4 | encoder currently not working |

The guided IMU calibration is intentionally operable from the web UI because the physical click inputs are still under diagnosis.
