...
CineMate uses independent services for the camera runtime, storage, networking, recovery and maintenance.
...
Provides the standalone maintenance interface on port 8080. It has no dependency on cinemate-autostart.service, Flask or Redis, so diagnostics remain available when the main camera application cannot start.

Read-only status, failure and log pages remain accessible for diagnosis. System-changing actions require the recovery token generated by the installer. If no token is configured, write actions are disabled.

See [Recovery console](recovery-console.md) for the operator workflow, security model, backup behaviour and config.txt confirm-or-revert mechanism.
