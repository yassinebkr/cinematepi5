...
## Local IMU / calibration keys

| Key | Written by | Description | Safe to change manually? |
| --- | --- | --- | --- |
| hud_level | Web UI / Cinemate | Enable HDMI LEVEL/SHAKE HUD | Yes |
| imu_roll | gyrologd | Calibrated camera-frame roll after operator zero | No |
| imu_pitch | gyrologd | Calibrated camera-frame pitch after operator zero | No |
| imu_shake | gyrologd | Phase B high-frequency gyro residual envelope | No |
| imu_motion | gyrologd | Smooth low-frequency angular-motion magnitude | No |
| imu_seq | gyrologd | Monotonic live-IMU publish sequence | No |
| imu_filter | gyrologd | Live filter implementation identifier, currently phaseB-v1 | No |
| imu_temp_c | gyrologd | ICM-42688 temperature | No |
| imu_calibrated | gyrologd | 1 when a saved physical calibration is active | No |
| imu_cal_active | Cinemate | 1 while the full-screen calibration wizard owns the display | Normally no |
| imu_cal_cmd | Cinemate/web UI | JSON command mailbox for the calibration engine | Advanced only |
| imu_cal_status | gyrologd | JSON calibration-state/status payload | No |
| imu_cal_notice | Cinemate | Short UI notice for calibration | No |
| imu_cal_selection | Cinemate | Result-screen selection state | Normally no |
| imu_cal_view_yaw | Cinemate | Calibration sphere view offset | Yes, diagnostic |
| level_zero_roll | gyrologd/Cinemate | Operator roll-zero reference | Use set zero |
| level_zero_pitch | gyrologd/Cinemate | Operator pitch-zero reference | Use set zero |
| last_take_shake | gyrologd | Legacy whole-take raw gyro magnitude summary | No |
